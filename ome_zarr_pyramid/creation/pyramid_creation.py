...
# def _get_cum_scale_factors_from_pyramid(input,
#                                         n_resolutions,
#                                         # scale_factor
#                                         ):
#     # input = pyr
#     # n_resolutions = 5
#     assert _is_pyramid(input)
#     shapes = np.vstack([input.array_meta[pth]['shape'] for pth in input.resolution_paths])
#     cum_scale_factors = shapes[0] / shapes
#     nlayers = n_resolutions
#     diff = nlayers - input.nlayers
#     for _ in range(diff):
#         newfactor = (cum_scale_factors[-1] / cum_scale_factors[-2]) * cum_scale_factors[-1]
#         newshape = np.round(shapes[0] / newfactor).astype(int)
#         shapes = np.vstack((shapes, newshape))
#         cum_scale_factors = np.vstack((cum_scale_factors, newfactor))
#     cum_scale_factors = shapes[0] / shapes
#     # print(cum_scale_factors)
#     # print(shapes)
#     return cum_scale_factors, shapes
...
class PyramidCreator:
    ...
    #     if _is_pyramid(self.input) and self.scale_factor is None:
    #         cum_scale_factors, shapes = _get_cum_scale_factors_from_pyramid(self.input, self.n_resolutions)
    #     else:
    #         scale_factors = np.vstack([self.scale_factor] * self.n_resolutions)
    #         scale_factors[0] = np.round(scale_factors[0] / scale_factors[0]).astype(int)
    #         cum_scale_factors = np.cumprod(scale_factors, axis=0)
    #         shapes = np.round(np.vstack([self.shape] * self.n_resolutions) / cum_scale_factors).astype(int) # get shapes in integer type first.
    #         cum_scale_factors = np.vstack([self.shape] * self.n_resolutions) / shapes
    #     scale_factors = {pth: tuple(scale_factor) for pth, scale_factor in zip(self.resolution_paths, cum_scale_factors)}
    #     scale = np.array(self.scale)
    #     scales = {pth: tuple(scale * scale_factor) for pth, scale_factor in
    #               zip(self.resolution_paths, cum_scale_factors)}
    #     shapes = {pth: tuple(shape.tolist()) for pth, shape in zip(self.resolution_paths, shapes)}
...
# inspect.signature(PyramidCreator)
# input_store = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data/filament.zarr"
# pyr = Pyramid().from_zarr(input_store)
# store1 = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data3/newarr1.zarr"

# creator = PyramidCreator(input = pyr,
#                          axis_order = 'zyx',
#                          n_resolutions = 3, scale_factor = (2, 2, 2), scale = (0.25, 0.25, 0.25),
#                          store = None
#                          )
# pyr1 = creator._create(method = 'create')

# creator = PyramidCreator(input = pyr.copy(),
#                          n_resolutions = 6,
#                          dtype = float,
#                          scale_factor = (2, 3, 3),
...
# creator = PyramidCreator(input = (1, 1, 100, 100, 100),
#                          axis_order = 'tczyx',
#                          n_resolutions = 1,
#                          # dtype = float,
#                          # scale_factor = (2, 3, 3),
...
# factors, scales, shapes = creator._parse_scale_factors()
# factors
# shapes
# pyr3 = creator.rand()
# pyr.layer_shapes
...
# pyr3[0][0:100]
#
# pyr3[0][0:100, 0:100, 0:100] = 3
# pyr3[0][0:100, 0:100, 0:100]
# pyr3 = creator._create(method = 'create')

# sample = zarr.zeros((300, 500), chunks = (50, 50), dtype = float, compressor = zarr.Zlib())
# sample = np.zeros((30, 50))
# store1 = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data3/newarr1.zarr"
# creator = ZarrayCreator(input = (300, 500), dtype = float, store = store1, chunks = (100, 100))
# z = creator.full(fill_value = 12)
# z[0:10, 0:10]
# import napari
# napari.view_image(z)
# creator1 = ZarrayCreator(sample)
# z1 = creator1.rand()
# np.mean(z1)

# store1 = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data3/newarr.zarr"
# creator1 = ZarrayCreator(sample, dtype = float, store = store1)
# z1 = creator1.zeros()
# store2 = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data3/newarr1.zarr"
# creator2 = ZarrayCreator(sample, dtype = float, store = store2, dimension_separator = '/')
# z2 = creator2.ones()
# h2 = zarr.open(store2)
# store3 = f"/home/oezdemir/PycharmProjects/dask_distributed/ome_zarr_pyramid/data3/newarr2.zarr"
# creator3 = ZarrayCreator(sample, dtype = float, store = store3, dimension_separator = '/')
# z3 = creator3.full(fill_value = int(3))
# h3 = zarr.open(store3)
#
